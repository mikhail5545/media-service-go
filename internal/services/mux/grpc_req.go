...
func (s *service) processAddBatch(ctx context.Context, asset *assetmodel.Asset, owners map[string][]string) error {
	addFunc := func(ctx context.Context, ownerType string, ids []string) (int64, error) {
		resp, err := s.VideoSvcClient.AddBatch(ctx, &videopb.AddBatchRequest{
	...
	return processBatchGRPC[videopb.AddBatchRequest](ctx, owners, addFunc)
}

func (s *service) processRemoveBatch(ctx context.Context, asset *assetmodel.Asset, owners map[string][]string) error {
	removeFunc := func(ctx context.Context, ownerType string, ids []string) (int64, error) {
		resp, err := s.VideoSvcClient.RemoveBatch(ctx, &videopb.RemoveBatchRequest{
			MediaServiceId: asset.ID,
			OwnerType:      ownerType,
			OwnerIds:       ids,
		})
	...
	return processBatchGRPC[videopb.RemoveBatchRequest](ctx, owners, removeFunc)
}

func (s *service) processChanges(ctx context.Context, asset *assetmodel.Asset, toAdd, toRemove map[string][]string) error {
	var allErrors []error
	if len(toAdd) > 0 {
	...
	if len(toRemove) > 0 {
		if err := s.processRemoveBatch(ctx, asset, toRemove); err != nil {
